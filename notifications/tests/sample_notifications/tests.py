...
class TestExtraDataCustomAccessor(NotificationTest):
    def setUp(self):
        self.from_user = User.objects.create_user(
            username="from_extra", password="pwd", email="example@example.com"
        )
        self.to_user = User.objects.create_user(
            username="to_extra", password="pwd", email="example@example.com"
        )
        notify.send(
            self.from_user,
    ...
    def test_extra_data(self):
        ...
        assert (
            notification
        ), "Expected a notification retrieved by custom extra data accessor"
        assert (
            notification.details == "test detail"
        ), "Custom accessor should return set value"
        assert (
            "details" not in notification.data
        ), "Custom accessor should not be in json data"
