...
class NotificationManagersTest(TestCase):
    ...
    def test_notify_send_return_val_group(
        self,
    ...
    def test_mark_all_as_read_manager_with_soft_delete(self):
        ...
        to_delete.save()
        self.assertTrue(
            Notification.objects.filter(recipient=self.to_user)
            .order_by("id")[0]
            .unread
        )
        Notification.objects.filter(recipient=self.to_user).mark_all_as_read()
    ...
    def test_mark_all_deleted_manager_without_soft_delete(
        ...
        self.assertRaises(
            ImproperlyConfigured, Notification.objects.mark_all_as_active
        )
    ...
    def test_mark_all_deleted_manager(self):
        ...
        self.assertEqual(
            Notification.objects.active().count(), self.message_count
        )
...
class NotificationTestPages(TestCase):
    ...
    def test_soft_delete_messages_manager(self):
        ...
        self.assertEqual(
            len(response.context["notifications"]),
            len(self.to_user.notifications.active()),
        )
        self.assertEqual(
            len(response.context["notifications"]), self.message_count - 1
        )
    ...
    def test_unread_list_api(self):
        ...
        data = json.loads(response.content.decode("utf-8"))
        self.assertEqual(
            sorted(list(data.keys())), ["unread_count", "unread_list"]
        )
        self.assertEqual(data["unread_count"], self.message_count)
    ...
    def test_unread_list_api_mark_as_read(
        self,
    ):  # pylint: disable=invalid-name
        self.login("to", "pwd")
        num_requested = 3
        ...
        data = json.loads(response.content.decode("utf-8"))
        self.assertEqual(
            data["unread_count"], self.message_count - 2 * num_requested
        )
        self.assertEqual(len(data["unread_list"]), num_requested)
    ...
    def test_unread_all_objects(self):
        ...
        self.assertEqual(
            notification["action_object_url"], f"foo/{action_customer.id}/"
        )
        self.assertEqual(
            notification["target_url"], f"bar/{target_object.id}/"
        )
...
class AdminTest(TestCase):
    ...
    def test_list(self):
        ...
        with CaptureQueriesContext(connection=connection) as context:
            response = self.client.get(
                reverse(
                    "admin:{0}_notification_changelist".format(self.app_name)
                )
            )
